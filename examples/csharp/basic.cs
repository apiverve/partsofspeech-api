...
{
    class PartsOfSpeechExample
    {
        private static readonly string API_KEY = Environment.GetEnvironmentVariable("APIVERVE_API_KEY") ?? "YOUR_API_KEY_HERE";
        private static readonly string API_URL = "https://api.apiverve.com/v1/partsofspeech";

        /// <summary>
        /// Make a GET request to the Parts Of Speech API
        /// </summary>
        static async Task<JsonDocument> CallPartsOfSpeechAPI()
        {
            try
            {
        ...
        {
            Console.WriteLine("📤 Calling Parts Of Speech API...\n");

            var result = await CallPartsOfSpeechAPI();
